...
bool InverseKinematics_TryMove(float Angle1, float Angle2, float Angle3, int PointX, int PointY, int PointZ)
{
  ...
  //accounts for power operation taken
  if(PointZ > 0)
  {
  	TargetAngle *= -1;
  }
  ...
  if(MovementLibraries_TestRange(MIDDLEJOINT, Angle2) && MovementLibraries_TestRange(TOPJOINT, Angle3))
  {
	  ...
	  nxtDisplayString(4, "Success");
	  writeDebugStream("Moved to Angle: %i", Angle1);
	...
	{
		nxtDisplayString(4, "Failed");
		writeDebugStreamLine("Failed: Angle out of Servo Range");
		return false;
	}
}
