...
float deltaT;
float a_x, a_y, a_z = 0; // accelerometer measurements
float w_x, w_y, w_z = 0; // gyroscope measurements in rad/s
float SEq_1 = 1.0;
float SEq_2 = 0.0;
float SEq_3 = 0.0;
...
task main()
{
  ...
  //if(!DIMUconfigGyro(DIMU, DIMU_GYRO_RANGE_500))	 //startup gyro
	...
	while(true) {
 			...
 			HTACreadAllAxes(Accelerometer, a_x, a_y, a_z);

 			// read gyro, degrees per second
 			w_z = HTGYROreadRot(Gyro);

 			writeDebugStreamLine("%f", a_x);
