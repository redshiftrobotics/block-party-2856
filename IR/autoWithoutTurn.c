...
task main()
{
	...
	while(true)
	{
		...
		if (slopeLOBF < 0) //the denominator can be the threshold turn point. Notice it is dependant on speed of robot and rotation of sensor.
		{
			...
			// turn right 90-ish deg.
			i = nMotorEncoder[left_back];
			while (nMotorEncoder[left_back] > i-(1440*0.9))
			{
				writeDebugStreamLine("comparing %i > %i", nMotorEncoder[left_back], i-(1440*1));
				motor[left_back] = -25;
				motor[left_front] = -25;
				motor[right_back] = -25;
				motor[right_front] = -25;
